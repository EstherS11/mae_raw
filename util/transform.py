...
class Compose(object):
    ...
    def __call__(self, image, label, edge):
        for t in self.segtransform:
            image, label, edge = t(image, label, edge)
...
class Crop(object):
    ...
    def __call__(self, image, label, edge):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            edge = cv2.copyMakeBorder(edge, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half,
                                      cv2.BORDER_CONSTANT, value=self.ignore_label)
        h, w = label.shape
