...
def read_annotations(data_path):
    ...
    for line in lines:
        ...
        if len(temp) == 1:
            ...
            sample_path, mask_path, _, label = temp
...
def plot_loss_curve(loss_train, loss_test, xlabel='Epoch', ylabel='Accuary(%)',title='',legend_label=['train', 'test'],
                     save_dir='results/figures',save_filename='Accuary_curve',fontsize=13):
    n=len(loss_train)
    x = np.arange(1, n+1, 1)
    fig = plt.figure()
...
# Input must be 0-1 float data
def best_f1_threshold(precision, recall, thresholds):
    best_f1=-1
    best_threshold=0.5
    for index in range(len(precision)):
        curr_f1=2.0*precision[index]*recall[index]/(precision[index]+recall[index]+1e-7)
        if best_f1<curr_f1:
            best_f1=curr_f1
