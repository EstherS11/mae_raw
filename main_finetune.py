...
def get_args_parser():
    ...
    parser.add_argument('--testdataDir3', default=r'/home/data1/zhangzr22/LLaVA_DATA/sft_forgery_data/autosplice', type=str,
                        help='choose the dir of dataset')
    parser.add_argument('--testdataDir4', default=r'/home/data1/zhangzr22/LLaVA_DATA/sft_forgery_data/IMD2020', type=str,
                        help='choose the dir of dataset')
...
def main(args):
    ...
    dataset_train = MyDataset(root_dir=args.dataDir, names_file=os.path.join(args.dataDir, args.train_txtdir),
                              crop_size=args.input_size, crop=False, transform=train_transform)
    ...
    if True:  # args.distributed:
        ...
        if args.dist_eval:
            if len(dataset_val1) % num_tasks != 0:
                print('Warning: Enabling distributed evaluation with an eval dataset not divisible by process number. '
                      'This will slightly alter validation results as extra duplicate entries are added to achieve '
                      'equal num of samples per-process.')
            sampler_val1 = torch.utils.data.DistributedSampler(
                dataset_val1, num_replicas=num_tasks, rank=global_rank, shuffle=True)  # shuffle=True to reduce monitor bias
            sampler_val2 = torch.utils.data.DistributedSampler(
                dataset_val2, num_replicas=num_tasks, rank=global_rank, shuffle=True)  # shuffle=True to reduce monitor bias
            sampler_val3 = torch.utils.data.DistributedSampler(
                dataset_val3, num_replicas=num_tasks, rank=global_rank, shuffle=True)  # shuffle=True to reduce monitor bias
            sampler_val4 = torch.utils.data.DistributedSampler(
                dataset_val4, num_replicas=num_tasks, rank=global_rank, shuffle=True)  # shuffle=True to reduce monitor bias
            sampler_val5 = torch.utils.data.DistributedSampler(
                dataset_val5, num_replicas=num_tasks, rank=global_rank, shuffle=True)  # shuffle=True to reduce monitor bias
            sampler_val6 = torch.utils.data.DistributedSampler(
                dataset_val6, num_replicas=num_tasks, rank=global_rank, shuffle=True)  # shuffle=True to reduce monitor bias
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch  % 2 == 0 or epoch + 1 == args.epochs:
            ...
            if test_stats1['loss']+test_stats2['loss']+test_stats3['loss']+test_stats4['loss'] <= min_loss:
                min_loss = test_stats1['loss']+test_stats2['loss']+test_stats3['loss']+test_stats4['loss']
                if args.output_dir:
            ...
            log_stats1 = {**{f'train_{k}': v for k, v in train_stats.items()},
                            **{f'test_{k}': v for k, v in test_stats1.items()},
                            'epoch': epoch,
                            'n_parameters': n_parameters}
            log_stats2 = {**{f'train_{k}': v for k, v in train_stats.items()},
                            **{f'test_{k}': v for k, v in test_stats2.items()},
                            'epoch': epoch,
                            'n_parameters': n_parameters}
            log_stats3 = {**{f'train_{k}': v for k, v in train_stats.items()},
                            **{f'test_{k}': v for k, v in test_stats3.items()},
                            'epoch': epoch,
                            'n_parameters': n_parameters}
            log_stats4 = {**{f'train_{k}': v for k, v in train_stats.items()},
                            **{f'test_{k}': v for k, v in test_stats4.items()},
                            'epoch': epoch,
                            'n_parameters': n_parameters}
            log_stats5 = {**{f'train_{k}': v for k, v in train_stats.items()},
                            **{f'test_{k}': v for k, v in test_stats5.items()},
                            'epoch': epoch,
                            'n_parameters': n_parameters}
            log_stats6 = {**{f'train_{k}': v for k, v in train_stats.items()},
                            **{f'test_{k}': v for k, v in test_stats6.items()},
                            'epoch': epoch,
                            'n_parameters': n_parameters}
            ...
            if args.output_dir and misc.is_main_process():
                if log_writer is not None:
                    log_writer.flush()
                with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                    f.write(json.dumps(log_stats3) + "\n")

            if args.output_dir and misc.is_main_process():
                if log_writer is not None:
                    log_writer.flush()
                with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                    f.write(json.dumps(log_stats4) + "\n")
            if args.output_dir and misc.is_main_process():
                if log_writer is not None:
                    log_writer.flush()
                with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                    f.write(json.dumps(log_stats5) + "\n")
            if args.output_dir and misc.is_main_process():
                if log_writer is not None:
                    log_writer.flush()
                with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                    f.write(json.dumps(log_stats6) + "\n")
            if args.output_dir and misc.is_main_process():
                if log_writer is not None:
                    log_writer.flush()
                with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
